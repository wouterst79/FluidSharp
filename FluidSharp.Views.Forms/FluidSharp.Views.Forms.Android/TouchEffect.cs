...
[assembly: ExportEffect(typeof(FluidSharp.Droid.TouchEffect), "TouchEffect")]
namespace FluidSharp.Droid
{
    public class TouchEffect : PlatformEffect
    {
        ...
        void FireEvent(TouchEffect touchEffect, int id, TouchActionType actionType, Point pointerLocation, bool isInContact)
        {
            ...
            // locaiton on device
            var pointondevice = new SKPoint((float)fromPixels(pointerLocation.X), (float)fromPixels(pointerLocation.Y));

            // Get the location of the pointer within the view
            touchEffect.view.GetLocationOnScreen(twoIntArray);
            double x = pointerLocation.X - twoIntArray[0];
            double y = pointerLocation.Y - twoIntArray[1];
            var pointinview = new SKPoint((float)fromPixels(x), (float)fromPixels(y));


            // Call the method
            onTouchAction(touchEffect.formsElement,
                new TouchActionEventArgs(id, actionType, pointondevice, pointinview, isInContact));
        }
    }
}